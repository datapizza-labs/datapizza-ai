import pytest
from datapizza.tools.filesystem import matches_patterns
...
def test_matches_regex(file_path, patterns, expected):
    result = matches_patterns(file_path, patterns)
    assert result == expected, f"Failed for {file_path} with patterns {patterns}"

def test_empty_patterns_list():
    assert matches_patterns("/any/path", []) == True, "Should return True with empty patterns list"